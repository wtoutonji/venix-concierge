...
function venix_concierge_home_content() {
	...
	return array(
		...
		'pillars' => array(
			array( $copy( 'Precision', 'Precyzja w każdym szczególe' ), $copy( 'Every detail considered before the journey begins.', 'Od odbioru po cel — każdy przejazd starannie skoordynowany.' ) ),
			array( $copy( 'Discretion', 'Dyskrecja, na której można polegać' ), $copy( 'Quiet, professional service without intrusion.', 'Prywatność i profesjonalizm są podstawą każdej usługi.' ) ),
			array( $copy( 'Reliability', 'Jasna komunikacja' ), $copy( 'Clear communication and dependable timing.', 'Przejrzyste informacje i czytelny proces zapytania.' ) ),
			array( $copy( 'Flexibility', 'Usługa, która się dostosowuje' ), $copy( 'Arrangements shaped around your schedule.', 'Gdy plany się zmieniają, zespół reaguje spokojnie.' ) ),
			array( $copy( 'Comfort', 'Indywidualne podejście' ), $copy( 'A considered environment from door to door.', 'Każde zapytanie prowadzimy zgodnie z indywidualnymi potrzebami.' ) ),
			array( $copy( 'Continuity', 'Międzynarodowe zrozumienie' ), $copy( 'One point of contact throughout.', 'Usługa dla klientów z różnych kultur i środowisk.' ) ),
		),
		'why' => array(
		...
		'form' => array(
			...
			'fields' => array(
				...
				array( 'home-phone', $copy( 'Phone / WhatsApp (optional)', 'Telefon / WhatsApp (opcjonalnie)' ), 'tel', 'phone', false, 'tel' ),
				array( 'home-date', $copy( 'Pickup Date', 'Data odbioru' ), 'date', 'pickup_date', true, '' ),
				array( 'home-passengers', $copy( 'Passengers (optional)', 'Pasażerowie (opcjonalnie)' ), 'number', 'passengers', false, '' ),
				array( 'home-from', $copy( 'Pickup Location', 'Miejsce odbioru' ), 'text', 'pickup_location', true, '' ),
				array( 'home-to', $copy( 'Destination (optional)', 'Cel podróży (opcjonalnie)' ), 'text', 'destination', false, '' ),
			),
